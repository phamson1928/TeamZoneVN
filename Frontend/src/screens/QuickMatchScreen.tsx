...
import { Zap, ArrowLeft, AlertCircle } from 'lucide-react-native';
...
export const QuickMatchScreen = () => {
  ...
  const { data: status, isLoading: isLoadingStatus } =
    useQuery<QuickMatchStatus>({
      ...
      refetchInterval: (query: any) =>
        query.state.data?.inQueue ? 5000 : false,
    });
  ...
  const getElapsedTime = (since?: string) => {
    ...
    const seconds = Math.floor(
      (new Date().getTime() - new Date(since).getTime()) / 1000,
    );
        ...
        {isLoadingProfiles || isLoadingStatus ? (
          ...
          <View style={styles.setupContainer}>
            ...
            {gameProfiles?.length > 0 ? (
              <View style={styles.gamesList}>
                {gameProfiles.map((p: any) => (
                  <TouchableOpacity
                    key={p.id}
                    style={[
                      styles.gameProfileCard,
                      selectedGameProfileId === p.id &&
                        styles.gameProfileCardActive,
                    ]}
                    onPress={() => setSelectedGameProfileId(p.id)}
                  >
                    <Image
                      source={{ uri: p.game.iconUrl }}
              ...
              <View style={styles.emptyWarning}>
                ...
                <Text style={styles.emptyText}>
                  Bạn chưa thêm Game Profile nào. Ghép đội cần thông tin Game để
                  tìm đồng đội.
                </Text>
              </View>
            )}
            ...
            <TouchableOpacity
              style={[
                styles.startBtn,
                !selectedGameProfileId && styles.startBtnDisabled,
              ]}
              disabled={!selectedGameProfileId || startMatchMutation.isPending}
              onPress={() =>
                selectedGameProfileId &&
                startMatchMutation.mutate(selectedGameProfileId)
              }
            >
              <LinearGradient
