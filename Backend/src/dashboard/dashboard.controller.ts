...
export class DashboardController {
  ...
  @ApiOperation({
    summary:
      '[Admin] Tổng quan dashboard — users, zones, groups, reports, growth',
  })
  @ApiResponse({
    status: 200,
  ...
  @ApiResponse({
    status: 200,
    description:
      'Mảng dữ liệu (date, count) để vẽ line/bar chart tăng trưởng user',
    schema: {
      example: {
        period: '7d',
  ...
  @ApiResponse({
    status: 200,
    description:
      'Mảng (gameId, gameName, count) để vẽ pie/bar chart phân bố zone theo game',
    schema: {
      example: {
  ...
  @ApiResponse({
    status: 200,
    description:
      'Mảng 24 phần tử (hour 0-23, count) để vẽ bar chart peak hours',
    schema: {
      example: {
        period: '7d',
  ...
  @ApiOperation({
    summary:
      '[Admin] Chart xu hướng tương tác xã hội (Likes & Friendships theo ngày)',
  })
  ...
  @Get('charts/reports')
  @ApiOperation({
    summary: '[Admin] Chart xu hướng báo cáo (Reports theo ngày)',
  })
  @ApiQuery({ name: 'period', required: false, enum: DashboardPeriod })
  ...
  @ApiOperation({
    summary: '[Admin] Chart mức độ tương tác (Zones & Groups mới)',
  })
  @ApiQuery({ name: 'period', required: false, enum: DashboardPeriod })
  ...
  @ApiOperation({
    summary: '[Admin] Chart thống kê xử lý vi phạm (Moderation)',
  })
  getModerationChart() {
