...
export class DashboardService {
  ...
  async getStats() {
    ...
    return {
      ...
      zones: {
        total: totalZones,
        open: openZones,
        full: totalZones - openZones,
      },
      groups: {
        total: totalGroups,
        active: activeGroups,
        dissolved: dissolvedGroups,
      },
      reports: {
        open: openReports,
        resolved: resolvedReports,
        total: openReports + resolvedReports,
      },
      growth: {
        newUsersToday,
        newUsersThisWeek,
  ...
  async getSocialEngagementChart(period: DashboardPeriod) {
    ...
    return {
      period,
      label: 'Xu hướng tương tác xã hội (Likes & Friendships)',
      data,
    };
  }
  ...
  async getLeaderboardTop() {
    const rows = await this.prisma.$queryRaw<
      { userId: string; likeCount: bigint }[]
    >`
    ...
    if (rows.length === 0)
      return { label: 'Top 10 Users theo số Like', data: [] };
  ...
  async getTopGamesChart() {
    const rows = await this.prisma.$queryRaw<
      { gameId: string; name: string; zoneCount: bigint; userCount: bigint }[]
    >`
            SELECT 
                g.id AS "gameId", 
                g.name, 
  ...
  async getModerationChart() {
    ...
    return {
      ...
      severityDistribution: severityCounts.map((r) => ({
        severity: r.severity,
        count: r._count,
      })),
    };
