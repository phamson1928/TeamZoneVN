...
export class ReportsService {
  ...
  async resolve(adminId: string, id: string, dto: UpdateReportDto) {
    ...
    if (report.targetType === ReportTargetType.ZONE) {
      ...
      if (!zone)
        throw new NotFoundException('Zone không tồn tại hoặc đã bị xóa');
      ...
      if (!group)
        throw new NotFoundException('Group không tồn tại hoặc đã bị xóa');
