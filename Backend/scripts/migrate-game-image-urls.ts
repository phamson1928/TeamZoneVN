 ...
 *
 * Run: npx tsx scripts/migrate-game-image-urls.ts
 */
...
async function main() {
  ...
  for (const game of games) {
    ...
    if (hasChange) {
      ...
      if (newBannerUrl !== game.bannerUrl)
        console.log(`   banner: ${newBannerUrl}`);
      updated++;
    } else {
  ...
  console.log(
    `   Upload them to: ${NEW_SUPABASE_URL}/storage/v1/object/public/game-assets/`,
  );
}
